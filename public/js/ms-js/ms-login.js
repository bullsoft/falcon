(function($, win, doc) {"use strict";
    ...
    Login = function(element, options) {
        ...
        if (this.options.templateId)
            this.template = $.MSspirit.template({
    ...
    $.extend(Login.prototype, {
        ...
        showDialog : function() {
            ...
            $('body').data('dialog').modal.modifyClosePosition({
                top : 10,
                right : 20
            });
        ...
        bindEvents : function() {
            ...
            $dialog.find('.unite-login').click(function() {
                ...
                if (!win.openWin)
                    win.openWin = {};
                if (win.openWin['unite-login'])
                    win.openWin['unite-login'].close();
        ...
        lastCheck : $.MSspirit.storage({
            nameSpace : 'bingbang'
        }).ini('lastLoginTime')
    });

    $(doc).on('click', '.ms-check-login', function() {
        var $this = $('body'), options = {}, data = MS.Utils.options($this.attr('data-ms-login'));
