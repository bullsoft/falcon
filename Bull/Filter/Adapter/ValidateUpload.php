...
class Bull_Filter_Adapter_ValidateUpload extends Bull_Filter_Adapter_Abstract
{
    /**                                                                                                       
     *                                                                                                        
     ...
     *                                                                                              
     */
    public function __invoke($value, $file_ext = null)
    {
        ...
        // has to be an array                                                                       
        if (! is_array($value)) {
            return $this->invalid('INVALID_UPLOAD_NOT_ARRAY');
        }
        ...
        if ($expect != $actual) {
            return $this->invalid('INVALID_UPLOAD_ARRAY_MALFORMED');
        }
        ...
        if ($value['error'] != UPLOAD_ERR_OK) {
            ...
            foreach ($this->_error_invalid as $error => $invalid) {
                if ($value['error'] == $error) {
                    $this->strInvalid = $invalid;
                    return false;
                }
            }
            // some other error                                                                           
            return $this->invalid('INVALID_UPLOAD_UNKNOWN_ERROR');
        }

        // is it actually an uploaded file?                                                               
        if (! is_uploaded_file($value['tmp_name'])) {
            // nefarious happenings are afoot.                                                            
            return $this->invalid('INVALID_UPLOAD_NOT_UPLOADED_FILE');
        }
        ...
        if ($file_ext) {
            ...
            return $this->invalid('INVALID_UPLOAD_FILENAME_EXT');
        }
