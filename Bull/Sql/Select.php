 ...
 * 
 */
class Bull_Sql_Select
{
    /**
     * 
     * A constant so we can find "ignored" params, to avoid func_num_args().
     * 
     * The md5() value of 'Bull\Sql\Select::IGNORE', so it should be unique.
     * 
     ...
     * 
     */
    const IGNORE = '--24f081a6ca7b57fd1ad4dc141a75da72';
     ...
     * 
     */
    public function __construct(Bull_Sql_Adapter_Abstract $sql)
    {
        $this->sql = $sql;
    }
    ...
    {
        if ($this->union) {
            return implode($this->split, $this->union) . $this->split . $this->toString();
        } else {
    ...
    protected function toString()
    {
        ...
        // open the statement
        if ($this->distinct) {
            $text = 'SELECT DISTINCT' . $this->split;
        } else {
            $text = 'SELECT' . $this->split;
        }
        
        // add columns
        if ($this->cols) {
            $text .= $line . implode($csep, $this->cols) . $this->split;
        }
        
        // from these sources
        if ($this->from) {
            $text .= 'FROM' . $line;
            $text .= implode($csep, $this->from) . $this->split;
        }
        
        // join these sources
        foreach ($this->join as $join) {
            $text .= $join . $this->split;
        }
        
        // where these conditions
        if ($this->where) {
            $text .= 'WHERE' . $line;
            $text .= implode($line, $this->where) . $this->split;
        }
        
        // grouped by these columns
        if ($this->group_by) {
            $text .= 'GROUP BY' . $line;
            $text .= implode($csep, $this->group_by) . $this->split;
        }
        
        // having these conditions
        if ($this->having) {
            $text .= 'HAVING' . $line;
            $text .= implode($line, $this->having) . $this->split;
        }
        
        // ordered by these columns
        if ($this->order_by) {
            $text .= 'ORDER BY' . $line;
            $text .= implode($csep, $this->order_by) . $this->split;
        }
        
        // modify with a limit clause per the adapter
        $this->sql->limit($text, $this->limit, $this->offset) . $this->split;
        
        // for update?
        if ($this->for_update) {
            $text .= "FOR UPDATE" . $this->split;
        }
    ...
    {
        $this->union[] = $this->toString() . $this->split . 'UNION';
        $this->reset();
        return $this;
    }
    ...
    {
        $this->union[] = $this->toString() . $this->split . 'UNION ALL';
        $this->reset();
        return $this;
    }
