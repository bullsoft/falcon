 ...
 * 
 */
abstract class Bull_Sql_Adapter_Abstract
{
    /**
     * 
     * A Bull_Sql_ColumnFactory for returning column information.
     * 
     * @var Bull_Sql_ColumnFactory
     * 
     */
    ...
    public function __construct(
        Bull_Sql_ProfilerInterface $profiler,
        Bull_Sql_ColumnFactory $column_factory,
        Bull_Sql_SelectFactory $select_factory,
        $dsn,
        $type,
        $username = null,
        $password = null,
     ...
     * 
     * @return Bull_Sql_ProfilerInterface
     * 
     */
     ...
     * 
     * @param string $spec The text of the SQL query, optionally with named
     * placeholders.
     * 
     ...
     * 
     */
    public function fetchAll($text, $data = array())
    {
        $this->checkReadSql($text);
     ...
     * 
     */
    public function fetchAssoc($text, array $data = array())
    {
        $this->checkReadSql($text);
        $stmt = $this->query($text, $data);
        $data = array();
     ...
     * 
     */
    public function fetchCol($text, array $data = array())
    {
        $this->checkReadSql($text);
        $stmt = $this->query($text, $data);
     ...
     * 
     */
    public function fetchValue($text, array $data = array())
    {
        $this->checkReadSql($text);
        $stmt = $this->query($text, $data);
     ...
     * 
     */
    public function fetchPairs($text, array $data = array())
    {
        $this->checkReadSql($text);
        $stmt = $this->query($text, $data);
        $data = array();
     ...
     * 
     */
    public function fetchOne($text, array $data = array())
    {
        $this->checkReadSql($text);
        $stmt = $this->query($text, $data);
     ...
     * 
     */
    public function delete($table, $cond, array $data = array())
    {
        $this->canWrite();
    ...
    protected function canWrite()
    {
        if ($this->type === "master")
        {
            ...
            throw new Bull_Sql_Exception_NoWriteOnSlave(Bull_Sql_Locale::get("ERR_NO_WRITE_ON_SLAVE"));
        }
    }

    protected function checkReadSql($text) { return true; }
    protected function isWriteSql($text)   { return false;}
}
